import { calculateHighwayRisk, classifySeverity, generateHighwayAlerts, summarizeMissionRisk, scenarioEvents } from './highwayRiskEngine.js';
...
const highwayVideo = document.getElementById('highway-video');
...
function renderEvents(events, sourceLabel = 'Scenario simulation') {
    if (!liveFeed || !riskScore || !riskSeverity || !recommendation || !missionMode || !battery || !missionSummary || !alertPanel)
        return;
    ...
    if (feedSource)
        feedSource.textContent = sourceLabel;
...
async function loadLiveModelEvents() {
    try {
...
async function startSimulation(scenario) {
    const toggleEl = document.getElementById('airsim-toggle');
    ...
    if (isLive) {
        if (feedSource)
            feedSource.textContent = "Connecting to Live Inference...";
        if (mjpegStream)
            mjpegStream.style.display = 'none';
        if (highwayVideo)
            highwayVideo.style.display = 'block';
        if (mockBg)
        ...
        try {
            livePollInterval = setInterval(async () => {
                ...
                if (feedSource)
                    feedSource.textContent = `Live Inference feed: ${scenario}`;
            }, 2000);
        }
        catch (e) {
            console.error(e);
            if (feedSource)
                feedSource.textContent = "Failed to connect to Inference.";
        }
    }
    else {
        if (mjpegStream)
        ...
        if (highwayVideo)
            highwayVideo.style.display = 'block';
        if (mockBg)
            mockBg.style.display = 'none'; // We use video instead of mockBg now
