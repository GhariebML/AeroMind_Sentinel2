...
function recommendationFor(event, severity) {
    if (event.type === "accident")
        return "Dispatch ambulance and traffic police; route drone to hold overhead view.";
    if (event.type === "pedestrian_on_highway")
        return "Alert nearest patrol unit and broadcast driver warning for pedestrian risk.";
    if (event.type === "blocked_lane")
        return "Notify road operator and recommend temporary lane closure guidance.";
    if (event.type === "stopped_vehicle")
        return "Send assistance vehicle and monitor rear-end collision risk.";
    if (event.type === "congestion")
        return "Increase coverage over upstream lanes and recommend traffic diversion.";
    if (event.type === "emergency_vehicle")
        return "Track access corridor and support priority routing.";
